...
class Round():
	...
	def create_first_round(PLAYERS):
		...
		for index in range(len(players)):
			...
			if index < demi_len:
				round.matches.append(([f"P{index}", 0], [f"P{index + demi_len}", 0]))
	...
	def define_pairs(cls, players, met_players):
		...
		while len(matches) < 4:
			...
			while j < len(players):
				if players[j][0] in met_players[players[i][0]] or j in set_indexes:
					j += 1
	...
	def test(cls):
		...
		for i in range(1, 7):
			...
			round = Round(ROUND.matches, ROUND.met_players, ROUND.players_with_id)
